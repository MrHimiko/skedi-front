// src/panels/user/plugins/integrations/services/CalendarSync.js
...
/**
 * Sync calendar events for a specific integration
 * Default is 3 months before and 3 months after today
 * 
 * @param {number} integrationId - Integration ID
 * @param {string} provider - Provider type (google_calendar, outlook_calendar)
 * @returns {Promise<boolean>} - Success status
 */
export async function syncSpecificCalendar(integrationId, provider) {
    try {
        // Show loading notification
        ...
        const endpoint = provider === 'outlook_calendar' ? 
            `user/integrations/${integrationId}/sync-outlook` : 
            `user/integrations/${integrationId}/sync`;
