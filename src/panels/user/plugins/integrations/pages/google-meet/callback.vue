<!-- src/panels/user/plugins/integrations/pages/google-meet/callback.vue -->
<script setup>
...
onMounted(() => {
    ...
    // Debug info
    details.value = `Code received: ${code ? 'Yes' : 'No'}, State: ${state ? state : 'None'}. Sending to parent window...`;
    console.log('OAuth callback received', { code, state, error });
    
    // Send message to opener window
    if (window.opener) {
        try {
            console.log('Trying to send message to parent window');
            
            // Get the current origin for a secure targetOrigin parameter
            // instead of using the wildcard "*" which is insecure
            const currentOrigin = window.location.origin;
            
            window.opener.postMessage({
                type: 'google_meet_oauth_callback', // Use this type to match the listener
                code,
                state
            }, currentOrigin);  // Use specific origin instead of wildcard "*"
            
            console.log('Message sent to parent window with target origin:', currentOrigin);
